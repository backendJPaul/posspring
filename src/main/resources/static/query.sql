...
begin
    select user.id, user.name, user.password, role.id as idRole, user.status from user inner join role on user.idRole = role.id where user.id = _id and user.status = _status;
end //
...
begin
    select user.id, user.name, user.password, role.name as role, user.status from user inner join role on user.idRole = role.id where user.name = _name and password = _password;
end //

call login("pticavilca","amnesiac");

call fetchUser(true);
call updateUser(7,"roxana","root",3);

select count(*) from user where user.name="pticlavilca" and password ="amnesiac";

call gotoUserId(true,36);